import React from "react";
import { HiArrowNarrowRight } from "react-icons/hi";
import me from "../assets/graduation photo.jpeg";
import { Link } from "react-scroll";

const Home = () => {
  return (
    <div name="home" className="h-screen w-full bg-[#0a192f]">
      <div className="max-w-screen-lg mx-auto flex flex-col items-center justify-center h-full px-4 md:flex-row">
        <div className="flex flex-col justify-center h-full">
          {/* <h2 className=""> */}
          <span className="animate-slide-in [animation-delay:200ms]">
          <h2 className="relative group text-4xl sm:text-7xl font-bold text-white">
            <span className="block transition-opacity duration-300 group-hover:opacity-0">
              Computer Science Major by day...
            </span>
            <span className="absolute top-0 left-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              Modern day Iron Man by night...
            </span>
          </h2>
          <p className="text-gray-500 py-4 max-w-md">
            As a Computer Science major at the University of Maryland, I
            translate theoretical concepts into real-world solutions, refine my
            problem-solving abilities, and advance my technical expertise,
            learning and working in cybersecurity and software engineering.
          </p>
          <div>
            <Link
              to="about"
              smooth
              duration={500}
              className="group text-white w-fit px-6 py-3 my-2 flex items-center rounded-md bg-gradient-to-r from-cyan-500 to-blue-500 cursor-pointer"
            >
              About Me
              <span className="group-hover:rotate-90 duration-300">
                <HiArrowNarrowRight size={25} className="ml-3" />
              </span>
            </Link>
          </div>
          </span>
        </div>
        <div>
          <img
            src={me}
            alt="my profile"
            className="rounded-2xl mx-auto w-2/3 md:w-full transition transform duration-700 ease-in-out]"
          />
        </div>
      </div>
    </div>
  );
};
export default Home;
